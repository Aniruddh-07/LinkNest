...
export function Chat() {
  ...
  const handleSendMessage = async (e: FormEvent) => {
    e.preventDefault();
    if (!inputValue.trim() || isAssistantThinking || isImageGenerating) return;
    ...
    // Check if the message triggers image generation
    else if (command.startsWith("/imagine ")) {
        setIsImageGenerating(true);
        const prompt = userMessageText.replace(/^\/imagine\s*/i, "");
        try {
            ...
            const errorMessage: Message = {
                user: "Assistant",
                text: "Sorry, I couldn't generate the image. Please try another prompt.",
                avatar: "https://placehold.co/40x40.png",
                hint: "robot error",
            };
            setMessages(prev => [...prev, errorMessage]);
  ...
  const isThinking = isAssistantThinking || isImageGenerating;
    ...
    <Card className="flex flex-col h-full">
      ...
      <CardContent className="flex-1 flex flex-col gap-4 p-6 pt-2">
            ...
            {messages.map((msg, index) => (
              <div key={index} className={`flex items-start gap-3 ${msg.user === 'You' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`rounded-lg px-3 py-2 text-sm max-w-[80%] ${msg.user === 'You' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                  ...
                  <p>{msg.text}</p>
                  {msg.imageUrl && (
                     <div className="mt-2 relative aspect-square w-full max-w-sm rounded-lg overflow-hidden border">
                        <Image src={msg.imageUrl} layout="fill" objectFit="cover" alt="Generated by AI" />
                    </div>
                  )}
                </div>
              </div>
            ))}
            {(isAssistantThinking || isImageGenerating) && (
              <div className="flex items-start gap-3">
                ...
                <div className="rounded-lg px-3 py-2 text-sm max-w-[80%] bg-muted">
                  ...
                  <p className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{isImageGenerating ? 'Creating image...' : 'Thinking...'}</span>
                  </p>
                </div>
              </div>
            )}
        ...
        <form onSubmit={handleSendMessage} className="relative">
          <Input 
            placeholder="Type... (/a to chat, /imagine to create)" 
            className="pr-20"
            value={inputValue}
          ...
          <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center">
            ...
            <Button type="submit" variant="ghost" size="icon" className="h-8 w-8" disabled={isThinking}>
